<h1>Bonjour, Je suis Rwane <br/> <a>Aspirant professionnel de la cybersécurité </a></h1>

<h2>🎓Certifations :</h2>

  - [Comptia security+]()  </br>
  - [BTL1]() </br>
  - [Soc Level 1, TryHackme]()
    

<h2>💻Cybersecurity  Projects:</h2>

- <b> Mémoire de fin d'étude</b>
  - [Conception et mise en œuvre d’un Systèmes de Défense Automatisés par l’IA pour la Cybersécurité](https://github.com/joshmadakor1/Sentinel-Lab)
  - [Etude et mise en place d'un outil d'adminstartion et supervision réseau](https://github.com/joshmadakor1/Sentinel-Lab)

- <b> Sécurité système – Surveillance & Gestion des Incidents </b>

  - [Configuratiion d'IAM & Mise en place d'un PAM]()
  - [Analyse et corrélation d’événements de sécurité avec des solutions SIEM ](https://github.com/joshmadakor1/AD_PS)
  - [FIM (File Integrity Monitor)](https://github.com/joshmadakor1/PowerShell-Integrity-FIM)
    
<h2>💻Administration Systèmes Projects:</h2>

  - [Mise ne place d’un serveur de sauvegarde & backup avec TrueNas](https://github.com/joshmadakor1/Jwipe.PowerShell)
  - [Installation & configuartion Windows Server : gestion AD, GPO, DNS, DHCP, NFS](https://github.com/joshmadakor1/AD_PS)
  - [FIM (File Integrity Monitor)](https://github.com/joshmadakor1/PowerShell-Integrity-FIM)


<h2>💻Audit Projects:</h2>

  - [Audit et sécurisation d’un Active Directory avec BloodHound ](https://github.com/joshmadakor1/Jwipe.PowerShell)
<h2> 🤳 Connect with me:</h2>

[<img align="left" alt="Rwane Nzinga | LinkedIn" width="22px" src="https://cdn.jsdelivr.net/npm/simple-icons@v3/icons/linkedin.svg" />][linkedin]


[linkedin]: https://www.linkedin.com/in/rwanenzinga/

<!--
**thecybersentinel1/thecybersentinel1** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
